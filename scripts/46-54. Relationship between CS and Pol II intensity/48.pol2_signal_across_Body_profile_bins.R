...
# plot histone and pol2 signal distribution across genome
if (T) {
  mks <- c("pol2", "ATAC", "H3K27ac", "H3K27me3", "H3K36me3", "H3K4me1", "H3K4me3", "H3K79me2", "H3K9me3")
      ...
      if (type == "single") {
        for (cell in c(cell1, cell2)) {
          ...
          for (mk in mks) {
            ...
            if (T) {
              qread_cell_dat <- function(cell, mk) {
                file <- paste0(dir1, "/48.", data_type, "_Body_", up_bin_num, "UP_", down_bin_num, "DW_", 
                               body_bin_num, "Body.bs", bin_size, ".", mk, ".mean_mat.", cell, ".single.qs")
                mat <- qread(file, nthreads = 6)
                
                # format the data
                dat <- get_cell_specific_body_plot_dat(mat, mat$gene_id)
                dat$cell <- cell
                
                return(dat)
              }
              
              dat <- qread_cell_dat(cell, mk)
            }
            ...
            if (T) {
              ...
              p <- plot_body_distribution(dat, title = paste0(cell, ": ", mk, " signal"), label_y = paste(mk, " Signal"), group = 4, group_col = col_cell, hide_legend = F)
            }
            
            # save the figure
            if (T) {
              file <- paste0(dir2, "/48.", mk, "_distribution_across_all_", data_type, "s_bs", bin_size, ".", cell, ".", type, ".jpeg")
      ...
      if (type == "merged") {
        ...
        for (mk in mks) {
          ...
          if (T) {
            ...
            dat1 <- qread_cell_dat(cell1, mk)
            dat2 <- qread_cell_dat(cell2, mk)
            dat <- rbind(dat1, dat2)
          ...
          # save the figure
          if (T) {
            file <- paste0(dir2, "/48.", mk, "_distribution_across_all_", data_type, "s_bs", bin_size, ".merged.", type, ".jpeg")
            ggsave(filename = file, plot = p, width = 8, height = 6)
          }
        }
      }
