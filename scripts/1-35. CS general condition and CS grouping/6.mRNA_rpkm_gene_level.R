...
# get rpkm expression level (gene level)
if (T) {
  # load the gene RPKM data
  if (T) {
  ...
  if (T) {
    file <- "results/2.pic/6.boxplot_expression_level_non0rpkm_gene.jpeg"
    ...
    pheatmap::pheatmap(cor(non0dat),
                       ...
                       filename = "results/2.pic/6.correlation_non0RPKM_gene.jpeg", width = 8, height = 6,
                       fontsize = 13)
  }
  
  # get average gene expression level: rpkm_med
  if (T) {
    colnames(rpkm)
    
    file <- "data/saved_data/6.median_rpkm_value_for_thp1_cd34_gene.qs"
    if (file.exists(file)) {
...
# qc for rpkm expression level (gene level)
if (T) {
  # density plot: with 0
  if (T) {
    summary(rpkm_med)
    ...
    if (T) {
      ...
      file <- "results/2.pic/6.gene_expression_level_with0_cumulative_curve_plot.jpeg"
  ...
  if (T) {
    ...
    if (T) {
      ...
      file <- "results/2.pic/6.gene_expression_level_without0_cumulative_curve_plot.jpeg"
      ggsave(file, plot = p, width = 8, height = 6)
    }
  }
...
# classify into 4 class (gene level)
for (cell in colnames(rpkm_med)) {
  ...
  if (T) {
    ...
    file <- paste0("results/2.pic/6.4_type_gene_expression_level_classification_", cell, ".jpeg")
    ggsave(file, plot = p, width = 8, height = 6)
  }
  ...
  if (T) {
    ...
    file <- paste0("data/saved_data/6.4_type_gene_expression_level_classification_", cell, ".qs")
