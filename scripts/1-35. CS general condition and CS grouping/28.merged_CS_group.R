  ...
  if (T) {
    ...
    if (T) {
      ggplot(data) +
        ...
        labs(title = "Scree Plot", x = "Principal Component", y = "Cumulative Variance Explained") +
        cowplot::theme_cowplot()
    }
  }
  ggsave(paste0("results/2.pic/28.elbowplot_gene.pdf"), width = 7, height = 7)
...
if (T) {
  ...
  ggsave(paste0("results/2.pic/28.elbowplot_emission.pdf"), width = 7, height = 7)
...
if (T) {
  ...
  ggsave(paste0("results/2.pic/28.clustree.pdf"), width = 7, height = 10)
...
if (T) {
  ...
  if (T) {
    ...
    ggsave(paste0("results/2.pic/28.states_cluster.pdf"), width = 7, height = 7)
  }
}
...
if (T) {
  ...
  emission.markers <- FindAllMarkers(dat,
                                     ...
                                     min.cells.feature = 1,
                                     min.cells.group = 1)
  write.table(emission.markers, file = paste0("results/1.tab/28.emission_differential_markers_info.csv"), 
              quote = F, sep = ",", col.names = T, row.names = F)
  
  VlnPlot(dat, features = sort(rownames(dat)))
  ggsave(paste0("results/2.pic/28.states_cluster_emission_features_vlnplot.jpeg"), width = 10, height = 10)
  
  FeaturePlot(dat, reduction = 'wnn.umap', features = sort(rownames(dat)))
  ggsave(paste0("results/2.pic/28.states_cluster_emission_features_pointplot.jpeg"), width = 10, height = 10)
  
  DoHeatmap(dat, features = emission.markers$gene)
  ggsave(paste0("results/2.pic/28.states_cluster_emission_features_heatmap.jpeg"), width = 10, height = 10)
}
...
if (T) {
  ...
  write.table(dists, file = file, quote = F, sep = ",", col.names = T, row.names = F)
}
...
if (T) {
  ...
  qsave(dat, file = "data/saved_data/28.seurat_merged_object.qs", nthreads = 6)
}
