...
if (T) {
  for (cell in cells) {
    ...
    if (! file.exists(file)) {
      ...
      gene_body_ID <- qread("data/saved_data/22.geneBody_metadata_tx_level_stratified_random_sampling.qs", nthreads = 6)
      
      c <- ifelse(cell == cell1, 1, 2)
      
      gene_body_ID <- gene_body_ID[! is.na(gene_body_ID[, paste0("cell", c, "_info")]), ]
      target_gene_pat <- gene_body_ID$tx_id
...
if (T) {
  file <- "data/saved_data/23.seurat_PC_resolution_parameters.qs"
  if (! file.exists(file)) {
    cell_pc_res <- lapply(cells, function(cell) {
      # cell <- cell1
      # read the raw data
      if (T) {
        # emission data
        ...
        gene <- qread(paste0("data/saved_data/23.seurat_gene_CS_percentage_input_mat_", cell, ".qs"), nthreads = 6)
      }
      ...
      pc_res <- sapply(seq(10, total_n_pc), function(pc) {
        ...
        if (T) {
          ...
          ggsave(paste0("results/2.pic/23.PC_resolution/", cell, "_PC", pc, "_cluster.jpeg"), width = 6, height = 10)
        }
    ...
    cell_pc <- do.call(rbind, cell_pc_res)
    cell_pc$res <- rep(paste0("res", resolutions), 2)
    
    qsave(cell_pc, file = file, nthreads = 6)
  }
  if (file.exists(file)) {
    cell_pc <- qread(file, nthreads = 6)
  }
  
  cell_pc$res <- gsub("res", "", cell_pc$res)
  cell_pc$res <- factor(cell_pc$res, levels = sort(as.numeric(unique(cell_pc$res))))
}
...
if (T) {
  for (cell in cells) {
    ...
    if (T) {
      cell_dat <- cell_pc[cell_pc$cell == cell, ]
    }
    
    # format the data
    if (T) {
      ggdat <- melt(cell_dat[, -grep("cell", colnames(cell_dat))], id.vars = "res")
    ...
    # save the figure
    ggsave(filename = paste0("results/2.pic/23.seurat_PC_resolution_parameters_", cell, ".pdf"), 
           plot = p, width = 10, height = 6)
  }
  rm(cell, p, ggdat, len, nrow, stat, yintercept, cell_dat)
}
...
if (T) {
  ...
  for (cell in cells) {
    ...
    file <- paste0("data/saved_data/23.all_PC_all_resolutions_chromIDEAS_clusering.", cell, ".qs")
...
if (T) {
  ...
  for (cell in cells) {
    ...
    if (T) {
      ...
      file <- paste0("results/2.pic/23.cor_withingroup_and_betweengroup_for_all_clustering_with_highest_resolutions_results.", cell, ".pdf")
...
# line1-47: 根据**分层抽样**挑出的基因，对基因每部分染色质状态占比数据进行准备
# line48-179: 因为在分群过程中参数PC数目和分辨率的选择尤为重要，会严重影响后续分析
#             故这里选择遍历最可能的PC数目和分辨率选择，考虑到seurat中使用的分辨率参数一般<1，故这里一共尝试0.1~1, 1.5, 2-5
#             在使用不同参数的前提下，同时将分群数目进行统计，以助PC数目和分辨率选择
#             分别独立对cd34和thp1进行分析，两者互不干扰，具体分析细节见script34
# line180-245: 将不同参数下所有染色质状态分类结果进行可视化展示
