...
if (T) {
  ...
  if (T) {
    ...
    file <- "results/2.pic/31.volcano_plot_for_all_gene_parts_differential_chromatin_states.jpeg"
...
if (T) {
  ...
  if (T) {
    ...
    ggdat$group <- str_split(rownames(ggdat), "[.]", simplify = T)[, 1]
    ggdat$cell <- str_split(ggdat$group, "@", simplify = T)[, 1]
    ggdat$cluster <- str_split(ggdat$group, "@", simplify = T)[, 2]
    ...
    ggdat$cell <- factor(ggdat$cell, levels = c("cd34", "thp1"))
  ...
  if (T) {
    ggdat$percentage <- ifelse(ggdat$cell == cell1,
                               ggdat$value / dat1$Total_num[match(ggdat$exp_group, dat1$mRNA)] * 100,
                               ggdat$value / dat2$Total_num[match(ggdat$exp_group, dat2$mRNA)] * 100)
  ...
  file <- "results/2.pic/31.expression_level_and_gene_location_for_CS_clusters.pdf"
  ggsave(file, plot = p, width = 10, height = 6)
  
  qsave(ggdat, file = "data/saved_data/31.cluster_mRNA_exp_loc_line_plot_mat.qs", nthreads = 6)
  rm(ggdat, col_fun, p)
}
...
if (T) {
  ...
  if (T) {
    ...
    ggdat <- lapply(levels(dat$cluster), function(c) {
      cell_cluster_loc_dat <- lapply(c(cell1, cell2), function(cell) {
        # test data
        if (F) {
        ...
        if (T) {
          subdat <- dat[dat$cluster == c & dat$cell == cell, ]
        }
      ...
      cell_cluster_loc_dat <- data.frame(do.call(rbind, cell_cluster_loc_dat))
        
      return(cell_cluster_loc_dat)
    })
  ...
  file <- "results/2.pic/31.expression_level_line_plot_for_CS_clusters.pdf"
  ggsave(filename = file, plot = p, width = 10, height = 6)
  
  qsave(ggdat, file = "data/saved_data/31.cluster_expresssion_mat.qs", nthreads = 6)
}
